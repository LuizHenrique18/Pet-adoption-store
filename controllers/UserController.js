...
module.exports = class UserController {
  static async register(req, res) {
    ...
    if (password !== confirmpassword) {
      res
        .status(422)
        .json({ message: "As senhas não são iguais, tente novamente!" });
      return;
    }
    ...
    if (userExists) {
      res.status(422).json({
        message: "Esse email já foi cadastrado! Tente um outro email.",
      });
    }
  ...
  static async login(req, res) {
    ...
    if (!user) {
      res.status(422).json({
        message:
          "Esse email não existe! Se você não tem uma conta, faço o cadastro!",
      });
      return;
    }
    ...
    if (!comparePassword) {
      res
        .status(422)
        .json({ message: "A senha está incorreta, tente novamente!" });
      return;
    }
  ...
  static async editUser(req, res) {
    ...
    if (user.email === email && userExists) {
      res
        .status(422)
        .json({ message: "Esse email já está em uso, utilize outro!" });
      return;
    }
    user.email = email;
